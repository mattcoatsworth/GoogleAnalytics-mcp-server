import { z } from 'zod';
    ...
    import { formatBatchPivotReportResponse } from '../utils/formatters.js';

    export const batchRunPivotReports = {
      schema: {
        ...
        })).describe("Array of pivot report requests to run in batch")
      },
      handler: async (params) => {
        try {
          ...
          const response = await analyticsData.properties.batchRunPivotReports(request);
          const formattedResponse = formatBatchPivotReportResponse(response.data);
          ...
          return {
            content: [{ type: "text", text: `Error running batch pivot reports: ${error.message}` }],
            isError: true
          };
        }
      },
      description: "Run multiple pivot reports in a single batch on Google Analytics data"
    };
